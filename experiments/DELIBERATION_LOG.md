...
## (2026-07-14) P6 계획 확정 — 사전등록 정합화 고민 + 신규시도 선별의 추론 여정
**왜 적나**: P6 착수 결정 자체보다, 계획 검증 중 사전등록·데이터에서 발견한 함정들과 그 해소 논리가 재사용 가치가 있어 기록.
- **함정 1 — "적격"의 이중 의미**: status.md의 "패널 적격 ≈6,700"은 (a) 편향 코호트 혼입 + (b) "raw 보유 ≥12" 기준이라 이중으로 과대. P6가 실제 쓰는 적격 = depth=-1 ∩ **유효 플레이 항목**(rel>0) ≥12 = **≈3,416**. build_relevance가 playtime 0 행을 버리므로 raw 보유와 다르다 — 패널 산정 때 이 구분을 놓치면 동결 시점에 N 부족으로 드러났을 것.
- **함정 2 — H1이 반증 불가였다**: v2 슬롯 표에 EASE가 없어서, 등록된 슬롯만 돌리면 "EASE가 격파한다"는 H1을 지지도 반증도 못 함(T36 정정 때 H1 텍스트만 고치고 표를 안 고친 불일치). 관측 전에 잡아서 A1 수정안으로 해소 — **사전등록도 감사 대상**이라는 교훈.
- **함정 3 — 패널 추출 조건의 은닉 편향**: 초안은 "위시리스트 보유자 중 추출"이었는데, 무편향 코호트의 위시 보유율이 41%(편향 코호트 97%)라 위시 조건 = 몰입 유저 필터 = 모처럼 확보한 무편향성 훼손. → 전체 적격에서 무조건 추출, 지표 B만 패널 내 부분집합(~58%)에서 계산. 검정력은 N=1,000으로 보전(B-부분 ≈580 > T34 n=179).
- **함정 4 — "시간분할"의 문자적 불가능**: owned에 취득 시각이 없어 "입력=컷오프 전 보유"는 재구성 불가. 이미 검증된 조작정의(정답=최근 ≤10 미보유 날짜有 위시, 입력=전체 플레이 프로필, 위시 입력 불사용=구조적 무누수)를 A5로 고정 — 새 정의 발명보다 T34 비교가능성 유지가 우선.
- **함정 5 — "패널 절반 편입" 혼합그래프**: 원안대로면 혼합 조건에서 패널 절반이 그래프에 들어가 순수 홀드아웃이 깨지고 paired 비교 검정력도 반감. → 혼합 = train ∪ 탐색풀(패널·예비와 결정적 분리)로 재정의(A4). 원안보다 엄격하게만 바뀌므로 관측 전 수정으로 정당.
- **신규시도 선별 논리**: 채택 기준 = "무편향 풀이어야만 가능한가 + 확정 패널을 오염 않는가". E1(시프트 정량화)·E2(무편향 인기도 prior — 스노볼 인기도의 왜곡을 실측으로 대체)·E3(중앙값 8게임 = 진짜 전형 유저, ≥12 필터 밖 — 판정 없는 기술통계 한정)·E4(포화곡선 = 크롤 중단 결정, 로드맵 약속). **기각**: 위시 ε-tier 입력(유일한 target-독립 축을 입력으로 쓰면 지표 B 자멸 위험), 소셜(실측 13엣지 = 사망), LambdaMART(T37 프록시 음성 + 기대값 낮음). E5는 H1 성립 시만 — 탐색풀 튜닝 승자는 reserve 500 + 신규 사전등록 경유(과적합-확정 함정 차단).
- **프로세스 메타(사용자 교정)**: P6 계획 수립에 Plan 에이전트를 기본 투입했다가 "계획에 독창성이 필요하냐" 지적 — 제약-고정 작업(사전등록 기반 조립)은 직접이 맞음. CLAUDE.md Delegation 절을 "명시적 독창성 필요 시만"으로 축소(커밋 97330d5). 단 이번 에이전트 산출물은 매몰비용 회수로 통합했고, 실제로 함정 1·3·4·5는 그 크로스체크가 잡아줌 — 위임의 가치는 '독창성'이 아니라 '독립 검증'에 있었다는 것도 함께 기록.

## (2026-07-14) P6 확정 직전·직후의 판단들 — 보충크롤 반증, H3 경계판정, 절대의미 사다리
**왜 적나**: 판정 수치는 JOURNAL T46에 있고, 여기엔 수치 밖의 판단 논리를 남긴다.
- **보충 크롤 전제 반증 → 중단 판단**: "미분류 1,013개 = 미수집"이라던 계획 전제가 실측(재시도 105콜 회수 0, 원시응답 200+`success:false`)으로 깨짐 — 전부 스토어에서 내려간 게임. 판단: 나머지 ~900콜은 기대값 0이므로 중단하고, 4.5% 풀 억제를 "구매 불가 게임의 정당한 제외"로 재해석(플레이타임 질량 기준 2.95%). 교훈: 승인된 계획 항목이라도 실행 중 전제가 반증되면 멈추고 기록하는 게 맞다.
- **H3 경계 판정(q=0.0494)의 처리**: β0.3(등록된 본명 knob)은 명확히 기각(wl ns + NDCG 해악 SIG). β0.2는 wl q=0.0494로 문자 그대로는 "유의 유지". 등록 문언대로면 knob 채택인데, (1) q가 경계선이고 (2) **EASE가 발굴축에서도 knob보다 강함**(0.0485 vs 0.0406) — knob의 존재 이유(발굴 보강)가 서빙 랭커 자체에 흡수됨. 판단: 문언 준수로 "β0.2의 B-우위는 SIG로 기록"하되, 서빙 채택은 하지 않음(지배당한 레버) — 이는 사후 재량이 아니라 "knob = userknn-계열 보조 레버"라는 등록 당시 정의 안에서의 무의미화임을 명시.
- **H2의 미세 반전**: "구별 불가" 예상과 달리 A축에서 pctl이 미세 유의 우세(−0.0025). 결론은 등록 규칙(B ns → 최단순)과 동일하게 pctl_game이라 판정 갈등 없음 — 예상 빗나감 자체를 숨기지 않고 기록.
- **사용자 질문이 만든 개선 2건**: ① "수치가 낮은데 신뢰 가능?" → 비교 판정 신뢰성(paired+null 분리)과 절대 품질 주장을 분리해 답하고, 절대 단위 병기(추천 20개당 위시 적중 0.33 vs POP 0.098)를 결과 보고 표준으로 채택. ② "절대적 의미를 가지려면?" → 4단계 사다리(절대 단위 병기 → 절대 루브릭 judge → 실인간 소표본 → 온라인) 중 1·2단계를 플랜에 정식 추가. 사용자 검토 게이트가 형식이 아니라 실제로 산출물을 바꿈.
- **"되돌릴 수 없음"의 정확한 의미 정리**(사용자 질문): 기술적 불가역이 아니라 통계적 불가역 — 패널은 첫 관측에서만 확정 증거. 재실행은 가능하지만 증거가치 0. 새 질문은 새 패널(예비 500 or 신규 추출)+새 사전등록으로.

## (2026-07-15) "결과가 예뻐 보였으면 좋겠다"(사용자) — 선 긋기와 이중 표기 정책
**요청**: 절대 평가 수치가 안 예뻐 보이니 "방법을 바꾸든 표기를 바꾸든" 예쁘게.
**그은 선**: 결과를 보고 방법을 바꾸는 것(지표가 예쁠 때까지 재측정·재정의)은 거절 — 이 프로젝트가 싸워온 사후 쇼핑이며 포트폴리오 차별점(결론 신뢰성) 자해. 정본 원수치·캐비앗은 불변.
**수용한 것**: 표기 계층 — "안 예쁨"의 실체가 계기 오차 섞인 원점수를 절대 점수처럼 쓰는 데 있었으므로, **기록된 수치의 산술 유도로만** 구성한 정직-우호 프레이밍을 채택(천장 정규화 97%, 구별-불가 서사, 우연 대비 49배, 관대 82% 등). **이중 표기 정책**: 헤드라인 프레이밍 옆에 원값 괄호 병기 + 캐비앗 부착 + 금지 목록(천장 없는 원점수, K@100%, 심층 꼬리 비교) 명문화 → `docs/portfolio-headlines.md` 신설, decisions.md 등재. 예쁨과 엄밀함이 충돌하지 않는 지점("천장까지 재서 정규화 보고")이 오히려 더 강한 스토리라는 판단.

## (2026-07-20) P5 재배선의 판단들 — 압축 사다리, 서빙에 숨어 있던 컷오프 3곳, 신규 유저 가중치
**왜 적나**: P5는 "확정 구성을 앱에 싣는" 조립 작업이었지만, 실행 중 나온 판단 4건이 재사용 가치가 있음.
- **EASE 영속화의 3중 딜레마**: 풀 스케일(23k유저) 밀집 적합 불가 / V-영속은 1.7GB 상주 / 무영속은 기동 수 분. 해법 = 12k 캡 적합 + **정확 B-행의 top-K 희소화**(E5 T-a 절제 실험의 청크 계산 코드를 서빙 빌더로 재목적화 — 실험 코드가 프로덕션 부품이 된 사례). K는 추측 대신 **게이트 사다리 실측**으로: 512(−0.0183)→1024(−0.0089)→2048(−0.0027 통과). 1024 탈락 시점에 합의된 사다리는 "V-폴백"이었으나 손실 반감 추세가 명확해 **사용자에게 1칸 연장을 제안·승인**받음 — "사다리 밖 결정은 사용자 몫" 원칙의 실전 적용.
- **컷오프 버그 계급의 세 번째 물결**: T35(평가)·T53 어댑터 설계에 이어, **서빙 코드 3곳**(hybrid._cf_ranked, agent_graph._interleave/seed_node)에 `score≤0 break`가 살아 있었음 — condcos(비음수)에선 무해해서 눈에 안 띄던 것. EASE 스왑 전에 어댑터 계약 검토 중 발견. 교훈: 랭커 교체는 "점수의 부호/스케일 가정"을 소비자 전부에서 감사해야 함(정규화 max-나눗셈도 같은 계급 — min-max로 교체).
- **신규 유저의 pctl_game**: 스냅샷 밖 유저는 per-game 백분위를 계산할 분포가 없음 → 게임별 21분위 ECDF 그리드를 아티팩트에 싣고 보간. 검증: 정확 pctl과 Spearman 0.996, e2e 차이 +0.0008 — 근사가 사실상 무손실임을 실측으로 확정하고 채택.
- **스모크가 잡은 의미 버그**: 시드 경로가 가중치 1.0을 "1분 플레이"로 ECDF 해석 → 점수 스케일 붕괴(순위는 보존). `score_with_weights` 명시 API로 분리. 스모크를 "실제 앱 코드 경로"로 설계한 것이 배당을 지불한 사례 — 단위 테스트만으론 계약 오해석을 못 잡음.

## (2026-07-22) 배포 검토가 잡아낸 유출 — graph_users.json의 SteamID 12,000개 공개 추적
**왜 적나**: "배포 전 공개물 감사"가 실제 유출을 잡은 사례 + 파생 아티팩트가 유저데이터 차단 규칙의 사각지대였다는 교훈.
- **발견 경위**: 온라인 배포(무료 호스팅) 타당성 검토 중 "공개 레포에 지금 뭐가 올라가 있나"를 감사하다가, P5 커밋(4171a7e, 07-20)이 `serving/data/ease/graph_users.json` — **원시 SteamID64 12,000개 평문 리스트** — 를 public 레포에 추적시킨 것을 발견. `steam.db`·export류는 gitignore로 막아뒀지만, **빌더가 내놓는 파생 아티팩트**는 규칙의 사각지대였다(같은 유저 식별자가 이름만 바꿔 나온 것).
- **판단**: 서빙 앱은 이 파일을 읽지 않는다(`ease_recommender`는 meta.json의 카운트만 소비; 실소비자는 오프라인 검증 `p5_validate`뿐) → **untrack + gitignore로 즉시 노출 중단(사용자 승인), 로컬 보존** — 앱 동작 무영향. 히스토리 완전 삭제(filter-repo + force push)는 파괴적 작업이라 별도 승인 대기. 노출 수위는 라이브러리 내용 없는 "이 계정들이 크롤 표본이었다"는 멤버십 정보로 한정적이나, 자체 규칙(크롤 유저데이터 비공개) 위반은 위반.
- **교훈**: 유저데이터 차단은 원천 파일이 아니라 **파생 경로 전체**에 걸어야 한다 — "빌더 출력에 식별자가 실리는가"가 커밋 전 체크 항목이어야 함(operations.md에 규칙 명문화).
- **후속 전수 스캔이 더 큰 건을 발견**: SteamID64 접두(7656119…) 패턴으로 추적 파일 전체를 `git grep` — **증거 JSON 7개가 추가 적발**: `p4_sweep/panels.json`(1,483 ID)·`p6_ood/p6_panels.json`(4,688 ID)·judge `unblind*.json` 5개(각 12–20 ID, 패널 유저↔추천 케이스 매핑이라 블라인드 payload와 결합 시 라이브러리 일부 재구성 가능). 코드 상수(STEAMID_BASE)·합성 예시 ID는 무해 판정. 7개 전부 동일 조치(untrack+gitignore, 로컬 보존 — 블라인드 payload/summary는 계속 추적하므로 증거 재현성 유지). "증거는 추적한다"는 관례가 "증거 안의 식별자"까지 정당화하지 않는다는 것 — 히스토리 퍼지 필요성은 이 발견으로 더 커짐(합계 고유 ID 수천 개).

## (2026-07-22) 배포의 본질 재정의(사용자 교정) + P7 재범위 + 운영 자동화 제기
- **배포 재정의**: 내가 "포트폴리오 증빙용 데모"로 좁혀 기술 상세(호스팅 사양·저장 방식) 위주로 설명하자 사용자가 교정 — 목표는 **깃에 온 사람이 실제로 체험하는 시스템 그 자체**(자기 스팀ID 입력 or 프롬프트 → 실제 추천). 설계 골격은 동일(무료 호스팅 + 키는 서버 비밀값 + 데모 picker 제거 + LLM 일일 쿼터 가드/무-LLM 강등)하나 프레이밍이 다름: '증빙물'이 아니라 '작동하는 공개 시스템'. 전달 실패 교훈: 기술 상세 전에 "방문자가 뭘 겪는가"부터 — 일상어 요약 규칙 재위반 사례로 기록.
- **P7 재범위(사용자 지시)**: "추천 품질을 올릴 남은 레버" 묶음으로 통합 — ① 세션 recency(`rtime_last_played`; 레포에 구현 흔적 0 — T12 학습 블렌드에 미포함, D-family recency는 업적 의존이라 업적 크롤 중단과 함께 사장) 사전등록 1회(귀무 기대), ② 콜드스타트/신작 표면(투타워의 유일 확정 니치 — 정확도 레버가 아니라 "co-play 없는 게임은 구조적으로 못 띄우는" 능력 공백 해소). 최대 *검증된* 품질 레버는 여전히 데이터 규모(E4 곡선 상승 중)로 P9 소관임을 병기.
- **운영 자동화(사용자 제기, DE 관점)**: "배포하면 수동 운영은 모순" — 타당. 후속 설계 대상: 크롤 스케줄러(기존 "OS 스케줄러 미사용" 결정의 재검토 필요 — 그 결정의 비용이 07-15→18 3일 공백이었음), 게임 차원 TTL 재크롤(30일 유통기한, 예산 ~8%), 아티팩트 월간 재빌드→게이트 통과 시 재배포. 단 크롤은 로컬 자원(2GB DB, ToU상 로컬 전용)이라 클라우드 CI가 아닌 **로컬 스케줄러 축**이 현실적.

## (2026-07-22) 루트 README 전면 개편 — "사람이 읽는 문서" 기준 확립(사용자 교정)
**왜 적나**: 개편 자체보다, 사용자가 명시한 "방문자용 문서" 스타일 기준이 향후 모든 인간-대상 문서에 적용될 규칙이라 기록.
- **문제 진단(사용자)**: 기존 README가 (1) 포트폴리오 증빙물처럼 읽힘 — 헤드라인 지표 나열, "정직한 포지셔닝" 류 자기 포지셔닝 멘트, (2) 변경사항 서술("재구축 완료", "이틀 새 반영")이 최종 문서에 침투, (3) 보안/약관처럼 방문자가 궁금해하지 않을 당연한 내용 포함, (4) 이모지·em-dash·과잉 볼드 등 AI 티 나는 표기.
- **확립된 기준**: 방문자용 문서 = 기본 설명 + 데모 진입 + 기본 로직 + 실행법만. 사람이 읽는 흐름으로, 문장은 짧고 자연스럽게, 목록성 내용은 구(句)로. 검증 서사·지표·피벗 역사는 docs/ 위키(results·evaluation)의 몫이지 README의 몫이 아님 — 정보의 존재가 아니라 **배치**가 문제였다는 것.
- **유지한 것**: doc-format 체커가 루트 README도 검사하므로 메타블록은 영문 소문자 한 줄(`> type: ...`)로 존치. 체커 96 docs 통과 확인.
