...
## seed — **FAIL**
- query: 다크소울 같은 거
- route: library (expected ['seed']) | cands 300 → filtered 300 | 127.4s
...
- route: library (expected ['multi_entity']) | cands 300 → filtered 300 | 126.2s
- failing checks: ['ok_route_ok']

## constraint_coop_kr — PASS
- query: 협동 가능하고 한국어 되는 게임
- route: library (expected ['anonymous', 'library']) | cands 300 → filtered 300 | 125.9s

## constraint_price — PASS
- query: 2만원 이하 협동 게임
- route: library (expected ['anonymous', 'library']) | cands 300 → filtered 300 | 126.4s

## anonymous_no_lib — **FAIL**
- query: 차분하고 분위기 좋은 인디 게임
- route: anonymous (expected ['anonymous', 'general']) | cands 0 → filtered 0 | 189.1s
