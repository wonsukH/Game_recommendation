...
- **살아남는 진짜 결과(BH-FDR 통과)**: **랭커가 선호보다 큰 레버 — 프로덕션 condcos는 양축 최하, 교체 대상.**
- **⚠️ 추가 정정(T35~T36)**: 감사가 잡은 **cutoff 버그**(`score<=0 break`가 EASE 선형점수 리스트를 조기 절단)를 제거하니 **EASE(λ≈100)가 정정된 랭커 승자** — dev NDCG 0.3189 vs userknn 0.2582(**+0.061 SIG**), 독립 wishlist서도 최상위권(0.0187). 즉 v6~T33의 "userknn이 최고 랭커"도 버그 아티팩트였음 → **서빙 랭커 권고 = EASE**(userknn/knnpd03 2군, condcos 최하). MF는 별개 확인상 버그 무관·실제 열세.

## TL;DR (교정본 v7.1)
P4의 **실측으로 방어되는 결과는 "랭커 교체"**입니다 — 현 프로덕션 co-play CF(condcos)는 양축 최하이고, 공정 재평가상 **EASE(닫힌형 선형 item모델, λ≈100)가 최강 랭커**(dev NDCG +0.061 SIG vs userknn, 독립축 최상위권). userknn/knnpd03이 그다음. *구 보고서의 "userknn 최고"·"EASE 열세"는 cutoff 버그 아티팩트였고 감사로 정정됨.* **선호 정의**는 pvalue/pctl 모두 무난하나 주 지표상 구별이 강건하지 않고(순환 타깃 한계), **knnpd03 인기패널티(구 "S0")는 측정된 승리가 아니라 발굴-강조 knob**(독립 wishlist선 +0.0073 SIG). games 크롤 완주(풀 39,743), 유저 2,694/10k. **최종 선택은 target-독립 주지표를 넣은 P6에서만.**

## 결정해주셔야 할 것 (교정·우선순위순)
1. **랭커(강건 결과, T36·T38 정정)**: **EASE(λ≈100)로 교체 권고** — 공정 재평가상 최강, **dev +0.061 SIG·fresh 무노출 n=854 +0.062 SIG**로 재현(userknn 상회, 독립축 최상위권, fit 0.9s). condcos(프로덕션)는 양패널 최하. userknn/knnpd03은 2군. *유의: EASE는 유저수³ 비용(현 1.1k=초)이라 10k서도 저렴; 최종 관문 P6 OOD.*
...
- **랭커 축(T36 공정 정정)**: **EASE(λ100) NDCG 0.3189 최강**(+0.061 SIG vs userknn) > knnpd03 0.2638 > userknn 0.2582 > **condcos 0.2175 최하**. 독립 wishlist: knnpd03 0.0191 ≈ EASE 0.0187 > userknn 0.0128 > condcos 0.0084. *구 "userknn≫ease"는 cutoff 버그(EASE 리스트 조기 절단) — 제거 후 역전.* **이게 P4의 실측 산출 = condcos 교체, EASE 서빙.**
