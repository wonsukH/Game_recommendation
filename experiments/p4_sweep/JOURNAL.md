...
**잔여 한계(정직)**: 경계값 0.25 부근(철권 0.25·DJMAX 0.25)은 여전히 통과 — 임계 상향은 과필터 위험과 트레이드오프, 실사용 피드백으로 재조정 여지. → **사용자 결정으로 0.30 상향(f53fbe3)**: 철권·DJMAX 탈락, ER 목록이 BR/MOBA/PvP로 수렴.

## [2026-07-22 11:10] T58 — **P8 종결: 동의 기반 5계정 라이브 데모 게시 + 데이터 윤리 라인 확정**
**게시 경로 논쟁의 결론(사용자와 3라운드)**: ① 크롤 타인 계정 게시 요청 → 재식별 지문(게임 조합=유일성)·ToU·자체 규칙 근거로 반대 ② 구성 페르소나 제안 → 사용자가 "신뢰 불가(체리피킹 검증 불능)" 반박(타당) ③ **동의받은 친구 계정**으로 합의 — 실존성+동의+검증가능성 모두 충족하는 최적해. 크롤-익명화 시안(무작위 B/C)은 폐기, "크롤 유저는 익명화해도 미게시" 각주를 공개 문서에 명문화(포트폴리오 가점 요소로 전환).
**데모 결과(본인+친구 3, 1명 비공개 제외 · `portfolio-headlines.md` 게시)**: 취향 표는 **엔진 가중치(백분위) 정렬**(사용자 지시 — "raw 시간이 아니라 우리가 의미 있게 쓴 지표로"). 하이라이트: 친구 1(림버스 718h)에게 ER 역추천, 친구 4의 Subnautica → **속편 Below Zero** 인지, 친구 3(파티/멀티) → MHW·Marvel Rivals·Helldivers 정합. 알려진 노이즈 정직 기록: 초저플레이 소품(무료 데모작 등)의 백분위 고평가.
**P8 최종 상태**: e2e 7/7 · 실인간 N=1 정합 · 시드 게이트 출시(0.30) · Gemini κ 교차 완료 · 데모 게시 — **로드맵 P8 DONE**. 잔여 = P7(선택)·P9(상시)·콜드스타트 백로그·포트폴리오 자료화.

## [2026-07-22 12:05] T59 — 가용성 필터 (실사용 검증이 잡은 2번째 결함 → 당일 수정)
**발단(사용자)**: 데모 하이라이트였던 Black Survival이 "스토어 검색에 안 나온다" → 스크린샷으로 **페이지 잔존형 판매중단** 실증("더 이상 Steam 상점에서 이용할 수 없습니다"). 내 1차 확인(appdetails success=True → "판매 중")은 **오판**이었음 — 페이지 존재 ≠ 구매 가능. 정정 기록.
**신호 발굴**: 크롤 시점 "price=None AND is_free=0" = 페이지 잔존형 판매중단 검출 신호. 풀 41,266 중 **2,391개(5.8%)** 해당, 소유자 상위 10개 전수 검증 결과 **전부 실제 유명 판매중단**(GTA V Legacy·Rocket League·Fall Guys·HITMAN 2·Forza Horizon 4·서비스종료 4종) — 오탐 없음. Black Survival도 크롤 시점에 이미 신호에 걸려 있었음(데이터는 알았는데 서빙이 안 썼던 것).
**수정**: `CatalogMeta.available()/availability_filter()` 신설 → 전 추천 출력 경로 적용(그래프 filter_node + hybrid warm/cold). ER 시드 목록에서 Black Survival·Ring of Elysium·Deceit(전부 서비스 종료/판매 중단) 탈락 확인. 스모크 PASS·pytest 39 green.
**한계·근본책(사용자 질문에 답변)**: 신호는 크롤 시점 스냅샷 — 3층 구조가 완성형: ① 크롤-시점 필터(금일 적용, 대량) ② **P9 TTL 재크롤**(games 페이즈에 fetched_at>30일 재방문 규칙, ~7.5k콜/일 — 백로그 등재) ③ 최종 top-k 서빙 시점 라이브 확인(캐시+TTL, 요청당 수 콜). 포트폴리오 문서의 Black Survival 하이라이트는 "결함 발견→수정" 서사로 교체(발굴 자랑보다 강한 스토리).
