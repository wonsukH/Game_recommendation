...
> **목표**: 이 레포의 문서를 **"미래의 Claude(에이전트)가 빠르고 정확하게 검색해 사용자에게 답하기 좋은 형태"** 로 표준화한다 — 사람 미관이 아니라 **검색·검수(retrieval) 최적화**가 1차 목적이다. **핵심 제약: 본문 내용·수치·문장은 절대 바꾸지 않는다(요약 금지). 서식·구조·분할만 표준화한다.**
...
3. **초점 단위 분할** — 1000줄 통독 대신 한 모듈만. 인덱스(허브)에서 라벨 보고 자식 1개만 연다.
4. **리포트는 결론(결정)을 맨 앞에(answer-first)** — 검색 시 첫 몇 줄에서 답(verdict+핵심 수치)을 얻는다.
5. **단일 진실원 + 상호링크** — 같은 사실이 여러 곳에서 어긋나지 않게(정본 1곳, 나머지는 링크).
...
> **결정(TL;DR)**: <한 줄 결론 + 핵심 수치>   ← 검색 시 맨 앞에서 답을 얻도록 결론을 위로
