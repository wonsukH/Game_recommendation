...
- **본인 자기평정(블라인드 아님)**: top-10 중 관심 7 · 모름 1 · 비선호 2 — LLM 심사 계기의 예측
  (관대 82%)과 정합.
- **실사용 피드백 → 당일 개선 2건**: ① "이터널 리턴 같은 거 → 유희왕?"(대중적 시드에서 co-play
  유사가 인구 차트로 퇴화) → 태그-유사도 게이트 추가 → 유희왕·리듬게임 탈락, SMITE(MOBA)·
  Battlerite 등 실제 유사 계열로 수렴. ② 사용자가 추천 목록의 한 게임이 **스토어에서 구매 불가**
  임을 발견(페이지 잔존형 판매중단) → 데이터의 "가격 없음+무료 아님" 신호가 이 계급을 정밀 검출
  함을 확인(GTA V Legacy·Rocket League·HITMAN 2 등 2,391종) → **가용성 필터**를 전 추천 경로에
