...
### run 3 — 스케줄 자동 실행 (`scheduled__2026-08-01T12:00:00+00:00`)

수동 개입 없이 cron이 만든 실행. 사람이 자리에 없는 동안 돌았다.

```
발화 2026-08-02 12:00:00.767 UTC   cron 경계 후 0.77초
종료 2026-08-02 12:27:04.294 UTC   27분 03초
```

| 태스크 | 결과 | 소요 |
|---|---|---|
| `behavioral_extract` | success | 10분 53초 |
| `build_catalog_db` | success | 2분 34초 |
| `build_ease_artifact` | success | 3분 39초 |
| `p5_validate` | success | 12분 14초 |
| `p5_smoke` | success | 12초 |

재시도 0회. 게이트 수치는 run 1·2와 신뢰구간까지 동일하고 스모크도 같은 결과다.
`logical_date`가 08-01 12:00인 것은 이 실행이 담당하는 구간의 시작이 거기이기 때문이고,
구간이 끝나는 08-02 12:00에 돌았다.

앞서 고친 두 결함이 이 실행에서 확인됐다.

| 항목 | 고치기 전 | run 3 |
|---|---|---|
| `catalog.json` | 라이브와 2,391건 불일치, 표준 JSON 아님 | 바이트 동일, 표준 JSON 통과 |
| `meta.json` 의 `git_head` | `"unknown"` | `a9512a6119417ff2944539a92704257cc38ded4d` (호스트 HEAD와 일치) |

실제 `serving/data`는 세 번의 전체 실행 뒤에도 `git status` 기준 변경 0이다.
...
EASE 쪽도 같은 방법으로 대조했다. 이쪽은 처음부터 일치했다.

| 산출물 | 결과 |
|---|---|
| `items.npy` | 바이트 동일 (34,050) |
| `avg_pt.npy` | 바이트 동일 |
| `pt_ecdf.npz` | `grid`·`qs` 두 배열 모두 동일 |
| `meta.json` | 실질 필드 전부 일치 (B_nnz 69,734,400 · cap 12000 · lam 100 · n_items 34,050 · seed · topk 2048) |

`meta.json`에서 다른 것은 출처 필드뿐인데, 그중 하나가 결함이었다.

```
git_head   live='3c55464334df0a8928233699ddc23cdcd38286c2'   dag='unknown'
```

`build_ease_artifact`는 `git rev-parse HEAD`로 커밋을 찍고 예외가 나면 `"unknown"`으로 떨어뜨린다.
Airflow 이미지에 git이 없어서 매번 그 경로를 탔다. 스크립트는 죽지 않으니 아무 신호도 없었고,
**이 스택이 만든 모든 산출물이 어떤 코드로 만들어졌는지 추적 불가능**한 상태였다.

이미지에 git을 넣고 `safe.directory`를 설정했다. 후자가 필요한 이유는 `/opt/project`가
root 소유로 마운트되는데 태스크는 uid 50000으로 돌아서 git의 소유권 검사에 걸리기 때문이다.
확인 결과 컨테이너가 호스트와 같은 HEAD를 반환한다.
...
- 스케줄러가 cron 시각에 스스로 실행을 만들고 파이프라인이 무인으로 완주한다 (run 3).
...
- (해소됨) `p5_rebuild`의 스케줄 발화는 run 3에서 확인했다.
...
- `parse_catalog_row`의 `currency` 가드. 지금은 컨테이너 pandas를 고정해 회피한 상태다.
