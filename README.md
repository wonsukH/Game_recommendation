...
### 🎮 **[→ 앱 바로 써보기 (라이브 데모)](https://game-rec-agent.streamlit.app/)**
> Steam 프로필 주소를 넣으면 내 플레이 기록 기반 개인화 추천을, 없으면 채팅으로 추천을 받습니다.
...
```
"스타듀 밸리 같은 거"  ·  "내 라이브러리 기반 추천"  ·  "새 장르 좀 발굴해줘"
...
| **seed** | "스타듀 밸리 같은 거" | co-play 유사작 + **태그-유사도 게이트**(대중 시드의 인기차트 퇴화 방지), 프랜차이즈 자동 제외 |
